...
function CheckoutPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await response.json(); if (!response.ok) return setStatus(result.details ? `${result.error} ${result.details}` : result.error || 'Order could not be created.'); setOrderId(result.orderId); setStatus('Order placed. Payment proof is awaiting verification.')
  }
